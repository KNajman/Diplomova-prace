...
architecture RTL of rgb2hsv is
    ...
    -- Maximální hodnota pro danou bitovou hloubku (např. 255 pro 8-bit)
    constant MAX_VAL : integer := (2 ** G_DATA_WIDTH) - 1;

    -- Měřítko pro HUE (Odpovídá 60 stupňům v 360 stupňovém kruhu mapovaném na MAX_VAL)
    -- Trik s (+ 3) zajistí správné matematické zaokrouhlení k nejbližšímu celému číslu.
    -- Pro 8-bit: (256 + 3) / 6 = 43.
    constant HUE_SCALE : integer := ((2 ** G_DATA_WIDTH) + 3) / 6;

    -- Offsety pro jednotlivé kanály (R=0°, G=120°, B=240°)
    constant OFFSET_R : unsigned(G_DATA_WIDTH - 1 downto 0) := to_unsigned(0, G_DATA_WIDTH);
    constant OFFSET_G : unsigned(G_DATA_WIDTH - 1 downto 0) := to_unsigned((2 ** G_DATA_WIDTH) / 3, G_DATA_WIDTH);
    constant OFFSET_B : unsigned(G_DATA_WIDTH - 1 downto 0) := to_unsigned((2 * (2 ** G_DATA_WIDTH)) / 3, G_DATA_WIDTH);
    ...
    type inv_rom_t is array (0 to 255) of unsigned(G_COEFF_FRACT_BITS + 1 downto 0);
    ...
    begin
        ...
        for i in 1 to 255 loop
            -- Vzorec: (2^FRACT_BITS + (i / 2)) / i  (Včetně zaokrouhlení)
    ...
    signal f1_user           : std_logic_vector(0 downto 0) := "0"; --tuser je 1bit sběrnice
    ...
    signal f4_s_mult         : unsigned((G_DATA_WIDTH * 2) + G_COEFF_FRACT_BITS + 1 downto 0);
    signal f4_h_mult         : signed((G_DATA_WIDTH * 2) + G_COEFF_FRACT_BITS + 4 downto 0);
        ...
        if rising_edge(clk) then
            if rst = '0' then           -- Active low reset 
                ...
                if pipe_en = '1' then
                    ...
                    v_max := maximum(maximum(v_r, v_g), v_b);
                    v_min := minimum(minimum(v_r, v_g), v_b);
                    ...
                    -- Diff počítáme se znaménkem (9 bitů pro 8-bit barvu, aby nedošlo k přetečení)
                    if (f1_max = f1_r) then
                    ...
                    -- Geniální trik: Pro adresování ROM vždy využijeme jen horních 8 bitů!
                    ...
                    -- Škálování Hue (Diff * 43) a Saturation (Delta * 255) zaručí precizní výsledek
                    ...
                    f4_s_mult <= f3_delta_scaled * f3_inv_max;

                    -- H_MULT = (Diff * 43) * InvDelta (Nezapomenout na konverzi InvDelta na signed!)
                    f4_h_mult <= f3_diff_scaled * signed('0' & f3_inv_delta);
                    ...
                    if f4_delta = 0 then
                        ...
                        -- Fyzický výběr bitů nahrazuje posun >> G_COEFF_FRACT_BITS
                        f5_s <= unsigned(f4_s_mult(G_COEFF_FRACT_BITS + G_DATA_WIDTH - 1 downto G_COEFF_FRACT_BITS));

                        -- Součet s offsetem automaticky přeteče (Wrap-Around), což elegantně vyřeší 
                        -- záporné úhly Hue bez nutnosti složitých IF podmínek!
                        f5_h <= f4_offset + unsigned(f4_h_mult(G_COEFF_FRACT_BITS + G_DATA_WIDTH - 1 downto G_COEFF_FRACT_BITS));
                    end if;
