...
--možná by šel použít tento trik
-- for i in 0 to KERNEL_SIZE * KERNEL_SIZE - 1 loop
--                     mult_res := resize(window(i / KERNEL_SIZE, i mod KERNEL_SIZE) * kernel_2d(KERNEL_SIZE - 1 - (i / KERNEL_SIZE), i mod KERNEL_SIZE), MULT_WIDTH);
--                     temp_sum := temp_sum + resize(mult_res, ACC_WIDTH);
--                 end loop;

        for i in 0 to (KERNEL_SIZE * KERNEL_SIZE) - 1 loop
            -- =============================================================
                -- STAGE 1: Násobení (Mult_regs) a vertikální flip jádra
                -- =============================================================
                ...
                -- vůči indexu 'r', tedy KERNEL_SIZE - 1 - r.
                    mult_regs(i) <= window(KERNEL_SIZE - 1 - (i / KERNEL_SIZE), i mod KERNEL_SIZE) 
                                  * kernel_2d(i / KERNEL_SIZE, i mod KERNEL_SIZE);
                end loop;
                    valid_pipe(1) <= valid_pipe(0);

                for i in 0 to KERNEL_SIZE * KERNEL_SIZE - 1 loop -- násobení všech prvků jádra s odpovídajícími pixely z okna, pro 3x3 to bude 9 iterací [0..8]¨
                    mult_res(i) = window(r, c) * kernel_2d(KERNEL_SIZE - 1 - r, c); -- Výsledek násobení pro každý prvek jádra a odpovídající pixel z okna
                end loop;


                -- =============================================================
                -- STAGE 2: Akumulace (Adder Tree) a Saturace
                -- =============================================================
                sum_var := (others => '0');
                for i in 0 to KERNEL_SIZE * KERNEL_SIZE - 1 loop
                    -- Tento cyklus Vivado automaticky zsyntetizuje jako vyvážený binární strom sčítaček
                    sum_var := sum_var + resize(mult_regs(i), ACC_WIDTH);
                end loop;

                -- Saturace a odeslání do výstupu
                if sum_var < 0 then
                    out_pixel_reg <= (others => '0');
                ...
                -- Předání validity na výstup
                valid_pipe(2) <= valid_pipe(1);

                -- Ošetření případu, kdy potrubí stojí (protože nemáme nová data),
            -- ale master modul nám v tomto taktu zrovna vyčetl platná data z výstupu.
            elsif pixel_out_ready = '1' and valid_pipe(2) = '1' then
                valid_pipe(2) <= '0';
        ...
        mac_sum <= accumulator;
                ...
                -- TODO: napsat pipelinovou MAC, násobení a akumulaci rozdělit do několika taktů, mezi nimi vložit registry pro udržení mezivýsledků, a řídit celý proces pomocí valid_pipeline signálů, které budou indikovat, kdy jsou data připravena pro další krok.
                -- násobičky nejsou takový problém jako scítačky, protože násobení je lokální operace, zatímco sčítání všech výsledků pro jeden pixel může způsobit kritickou cestu.
    -- 1. varianta přes skip buffer, ten který je přes stavový automat - dělá "zácuky"

                -- FIFO na vstup i na výstup, celý výpočet řídit ready signálem FIF
