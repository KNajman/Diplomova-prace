...
void hls_filter_median_3x3(hls::stream<axis_gray> &s_axis_video,
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
  ...
  // Je vytvořen vlastní kruhový buffer pro uchování předchozích řádků
  // (Inferováno jako BRAM).
  ap_uint<PIXEL_WIDTH> line_buffer[KERNEL_SIZE - 1][MAX_IMG_WIDTH];
#pragma HLS ARRAY_PARTITION variable = line_buffer complete dim = 1

  // Je vytvořen vlastní posuvný registr pro výpočetní okno (Inferováno jako
  // klopné obvody).
...
#pragma HLS UNROLL factor = 3
      sort_three(window[row][0], window[row][1], window[row][2], min[row],
                 med[row], max[row]);
    }
    ...
    sort_three(max_of_mins, med_of_meds, min_of_maxs, final_min, final_med,
               final_max);
    ...
    // Výstupní pixel je validní pouze pokud je okno plně naplněno daty (okraje
    // ignorovány).
