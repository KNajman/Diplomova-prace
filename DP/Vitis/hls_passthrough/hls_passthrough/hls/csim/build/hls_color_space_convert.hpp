...
struct axi_stream_video { // Podle Xilinx AXI4-Stream Video Protocol XAPP793
  ...
  ap_uint<DATA_WIDTH/8> keep;  // PŘIDÁNO: Zajišťuje platnost bajtů (potřebné pro AXI standard)
...
using axis_video_dma = axi_stream_video<AXI_DMA_WIDTH>;

/*
 Univerzální struktura pro pixel v libovolném barevném prostoru, pokud se nezvolí jinak počítám s 8 bit na barvu a 3 kanály
*/
...
/* Z RGB do HSV a zpět - tyto funkce jsou složitější kvůli nelineární povaze
 ...
 obrazů v paměti.*/
void hls_rgb_2_hsv(hls::stream<axis_video_dma> &s_axis_rgb, hls::stream<axis_video_dma> &m_axis_hsv);


/* Zpětná neztrátová konverze z HSV do RGB, která zachovává přesnost a
/ minimalizuje využití hardwarových prostředků.
*/
void hls_hsv_2_rgb(hls::stream<axis_video_dma> &s_axis_hsv, hls::stream<axis_video_dma> &m_axis_rgb);
