...
entity hls_histogram_gray_control_s_axi is
generic (
...
end entity hls_histogram_gray_control_s_axi;
...
-- 0x01c : reserved
-- 0x400 ~
-- 0x7ff : Memory 'hist_out' (256 * 32b)
--         Word n : bit [31:0] - hist_out[n]
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of hls_histogram_gray_control_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_HIST_OUT_BASE : INTEGER := 16#400#;
    constant ADDR_HIST_OUT_HIGH : INTEGER := 16#7ff#;
    ...
    -- memory signals
    signal int_hist_out_address0 : UNSIGNED(7 downto 0);
    signal int_hist_out_ce0    : STD_LOGIC;
    signal int_hist_out_be0    : UNSIGNED(3 downto 0);
    signal int_hist_out_d0     : UNSIGNED(31 downto 0);
    signal int_hist_out_address1 : UNSIGNED(7 downto 0);
    signal int_hist_out_ce1    : STD_LOGIC;
    signal int_hist_out_q1     : UNSIGNED(31 downto 0);
    signal int_hist_out_read   : STD_LOGIC;
    signal int_hist_out_write  : STD_LOGIC;

    component hls_histogram_gray_control_s_axi_ram is
        generic (
    ...
    end component hls_histogram_gray_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_hist_out
int_hist_out : hls_histogram_gray_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "S2P",
     BYTE_WIDTH => 8,
...
port map (
     clk0       => ACLK,
     address0   => int_hist_out_address0,
     ce0        => int_hist_out_ce0,
     we0        => int_hist_out_be0,
     d0         => int_hist_out_d0,
     q0         => open,
     clk1       => ACLK,
     address1   => int_hist_out_address1,
     ce1        => int_hist_out_ce1,
     we1        => (others=>'0'),
     d1         => (others=>'0'),
     q1         => int_hist_out_q1);
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_hist_out_read = '0') else '0';
    RVALID    <= RVALID_t;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                ...
                elsif (int_hist_out_read = '1') then
                    rdata_data <= RESIZE(int_hist_out_q1, 32);
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- hist_out
    int_hist_out_address0 <= RESIZE(UNSIGNED(hist_out_address0), 8);
    int_hist_out_ce0     <= hist_out_ce0;
    int_hist_out_be0     <= (others => hist_out_we0);
    int_hist_out_d0      <= RESIZE(UNSIGNED(hist_out_d0), 32);
    int_hist_out_address1 <= raddr(9 downto 2);
    int_hist_out_ce1     <= '1' when ar_hs = '1' else '0';
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ar_hs = '1' and raddr >= ADDR_HIST_OUT_BASE and raddr <= ADDR_HIST_OUT_HIGH) then
                    int_hist_out_read <= '1';
                else
                    int_hist_out_read <= '0';
                end if;
...
entity hls_histogram_gray_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity hls_histogram_gray_control_s_axi_ram;

architecture behave of hls_histogram_gray_control_s_axi_ram is
