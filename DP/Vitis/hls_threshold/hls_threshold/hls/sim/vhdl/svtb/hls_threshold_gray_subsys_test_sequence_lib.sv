...
//
//==============================================================
`ifndef HLS_THRESHOLD_GRAY_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define HLS_THRESHOLD_GRAY_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
    `define AUTOTB_TVIN_s_axis_video_s_axis_video_TDATA  "../tv/cdatafile/c.hls_threshold_gray.autotvin_s_axis_video.dat" 
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class hls_threshold_gray_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "hls_threshold_gray_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(hls_threshold_gray_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(hls_threshold_gray_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            if (!uvm_config_db#(hls_threshold_gray_reference_model)::get(p_sequencer,"", "refm", refm))
                ...
                begin
                    fork
                        begin
                            ...
                            if( refm.hls_threshold_gray_cfg.port_s_axis_video_cfg.prt_type == AP_VLD ) wait(refm.misc_if.tb2dut_ap_start === 1'b1);
                        ...
                        begin
                            ...
                            for(int i=0; i<3; i++) begin
                                logic[63:0] data64bit_thresh_val[$];
                                logic[32-1:0] databusbit_thresh_val[$];
                                logic[63:0] data64bit_thresh_type[$];
                                logic[32-1:0] databusbit_thresh_type[$];
                                logic[63:0] data64bit_width[$];
                                logic[32-1:0] databusbit_width[$];
                                logic[63:0] data64bit_height[$];
                                logic[32-1:0] databusbit_height[$];
                                data64bit_thresh_val.delete(); databusbit_thresh_val.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                refm.mem_blk_pages_control_thresh_val.tobusdata(data64bit_thresh_val, refm.mem_blk_pages_control_thresh_val.rd_page_idx, 32);
                                foreach(data64bit_thresh_val[s]) databusbit_thresh_val[s]=data64bit_thresh_val[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_thresh_val, 0, 16, 1);
                                data64bit_thresh_type.delete(); databusbit_thresh_type.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                refm.mem_blk_pages_control_thresh_type.tobusdata(data64bit_thresh_type, refm.mem_blk_pages_control_thresh_type.rd_page_idx, 32);
                                foreach(data64bit_thresh_type[s]) databusbit_thresh_type[s]=data64bit_thresh_type[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_thresh_type, 0, 24, 1);
                                ...
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_width, 0, 32, 1);
                                ...
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_height, 0, 40, 1);
