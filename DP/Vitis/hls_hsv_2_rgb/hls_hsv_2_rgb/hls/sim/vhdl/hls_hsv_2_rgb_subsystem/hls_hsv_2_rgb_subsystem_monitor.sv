...
`ifndef HLS_HSV_2_RGB_SUBSYSTEM_MONITOR_SV
`define HLS_HSV_2_RGB_SUBSYSTEM_MONITOR_SV
...
class hls_hsv_2_rgb_subsystem_monitor extends uvm_component;

    hls_hsv_2_rgb_reference_model refm;
    hls_hsv_2_rgb_scoreboard scbd;

    `uvm_component_utils_begin(hls_hsv_2_rgb_subsystem_monitor)
    `uvm_component_utils_end

    uvm_analysis_imp_svr_master_in_stream#(svr_pkg::svr_transfer#(32), hls_hsv_2_rgb_subsystem_monitor) svr_master_in_stream_imp;
    uvm_analysis_imp_svr_slave_out_stream#(svr_pkg::svr_transfer#(32), hls_hsv_2_rgb_subsystem_monitor) svr_slave_out_stream_imp;
    uvm_analysis_imp_axi_wtr_control#(axi_pkg::axi_transfer, hls_hsv_2_rgb_subsystem_monitor) control_wtr_imp;
    uvm_analysis_imp_axi_rtr_control#(axi_pkg::axi_transfer, hls_hsv_2_rgb_subsystem_monitor) control_rtr_imp;

    virtual function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        if (!uvm_config_db#(hls_hsv_2_rgb_reference_model)::get(this, "", "refm", refm))
            `uvm_fatal(this.get_full_name(), "No refm from high level")
        `uvm_info(this.get_full_name(), "get reference model by uvm_config_db", UVM_MEDIUM)
        scbd = hls_hsv_2_rgb_scoreboard::type_id::create("scbd", this);
    endfunction
